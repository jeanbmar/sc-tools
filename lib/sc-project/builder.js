...
const TextFieldOriginal = require('../schemas/tags/text-field-original');
const Matrix2x3 = require('../schemas/tags/matrix-2x3');
const ColorTransformation = require('../schemas/tags/color-transformation');
const MovieClipOriginal = require('../schemas/tags/movie-clip-original');
const MovieClipFrame = require('../schemas/tags/movie-clip-frame');
const ScalingGrid = require('../schemas/tags/scaling-grid');
const Tag41 = require('../schemas/tags/tag-41');
const Timeline = require('../schemas/tags/timeline');
const MovieClipModifierOriginal = require('../schemas/tags/movie-clip-modifier-original');
const Tag38 = require('../schemas/tags/tag-38');
const Tag42 = require('../schemas/tags/tag-42');
...
class Builder {
    ...
    static async scp2sc(inputProjectDirectory, outputDirectory) {
        ...
        const projectConfig = JSON.parse(readFileSync(join(inputProjectDirectory, `${name}.conf`)));
        ...
        texturePaths.forEach((texturePath) => {
            ...
            if (png.ctype !== pixelInfo.colorType) {
                console.warn(`WARNING: The given colorType (${png.ctype}) doesn't match with desired colorType (${pixelInfo.colorType})`);
                console.warn('WARNING: The .sc file may not work as intended!!');
                console.warn("WARNING FIX: Either convert the image format to desired colorType (or) change the 'pixelCode' correctly in .conf file in order to match with desired colorType.");
            }
        ...
        sc.textFields = textFieldPaths.map((textFieldPath) => {
            const textField = new TextFieldOriginal();
            const textFieldName = basename(textFieldPath, '.conf');
            const textFieldConfig = JSON.parse(readFileSync(join(textFieldsDirectory, `${textFieldName}.conf`)));
        ...
        // export matrices
        const matricesFile = join(inputProjectDirectory, `${inputProjectDirectory}_matrices.conf`);
        ...
        const colorTransformationsDirectory = join(inputProjectDirectory, 'color_transformations');
        const colorTransformationPaths = glob.sync(join(colorTransformationsDirectory, '*.conf'));
        sc.colorTransformations = colorTransformationPaths.map((colorTransformationPath) => {
            const colorTransformation = new ColorTransformation();
            const colorTransformationName = basename(colorTransformationPath, '.conf');
            const colorTransformationConfig = JSON.parse(readFileSync(join(colorTransformationsDirectory, `${colorTransformationName}.conf`)));
        ...
        sc.movieClips = movieClipPaths.map((movieClipPath) => {
            const movieClip = new MovieClipOriginal();
            const movieClipName = basename(movieClipPath, '.conf');
            const movieClipConfig = JSON.parse(readFileSync(join(movieClipsDirectory, `${movieClipName}.conf`)));
            ...
            movieClip.frameDataLength = movieClip.tagSignature === 14 ? 14 : movieClip.frameData.length;
            ...
            movieClip.scalingGrids = movieClipConfig.scalingGrids.map((scalingGridConfig) => {
        ...
        sc.timelines = timelinePaths.map((timelinePath) => {
            const timeline = new Timeline();
            const timelineName = basename(timelinePath, '.conf');
            const timelineConfig = JSON.parse(readFileSync(join(timelinesDirectory, `${timelineName}.conf`)));
        ...
        const modifiersDirectory = join(inputProjectDirectory, 'movie_clip_modifiers');
        const modifierPaths = glob.sync(join(modifiersDirectory, '*.conf'));
        sc.movieClipModifiers = modifierPaths.map((modifierPath) => {
            const modifier = new MovieClipModifierOriginal();
            const modifierName = basename(modifierPath, '.conf');
            const modifierConfig = JSON.parse(readFileSync(join(modifiersDirectory, `${modifierName}.conf`)));
        ...
        sc.tag38s = tag38Paths.map((timelinePath) => {
            const tag38 = new Tag38();
            const tag38Name = basename(timelinePath, '.conf');
            const tag38Config = JSON.parse(readFileSync(join(tag38sDirectory, `${tag38Name}.conf`)));
        ...
        sc.tag42s = tag42Paths.map((timelinePath) => {
            const tag42 = new Tag42();
            const tag42Name = basename(timelinePath, '.conf');
            const tag42Config = JSON.parse(readFileSync(join(tag42sDirectory, `${tag42Name}.conf`)));
