...
import ShapeOriginal from './tags/shape-original.mjs';
import MovieClipOriginal from './tags/movie-clip-original.mjs';
import TextFieldOriginal from './tags/text-field-original.mjs';
import SwfTexture from './tags/swf-texture.mjs';
import Matrix2x3 from './tags/matrix-2x3.mjs';
import ColorTransformation from './tags/color-transformation.mjs';
import Timeline from './tags/timeline.mjs';
import Flag from './tags/flag.mjs';
import MovieClipModifierOriginal from './tags/movie-clip-modifier-original.mjs';
import Tag38 from './tags/tag-38.mjs';
import Tag42 from './tags/tag-42.mjs';
...
class SupercellSC {
  ...
  decode(smartBuf) {
    ...
    exportIds.forEach((exportId) => {
      this.exports.push({ exportId, exportName: smartBuf.readAscii() });
    });
    while (smartBuf.remaining()) {
      ...
      switch (tagSignature) {
        ...
        case 37:
          this.movieClipModifiers.push(
            MovieClipModifierOriginal.decode(smartBuf)
          );
          break;
        case 38:
        case 39:
      ...
      if (before !== smartBuf.readOffset - tagSize) {
        throw new Error(
          `inconsistent tag size for tag signature ${tagSignature} (expected ${tagSize}, read ${
            smartBuf.readOffset - before
          })`
        );
      }
    }
  ...
  encode(smartBuf) {
    ...
    this.colorTransformations.forEach((colorTransformation) =>
      colorTransformation.encode(smartBuf)
    );
    this.movieClips.forEach((movieClip) => movieClip.encode(smartBuf));
    this.timelines.forEach((timeline) => timeline.encode(smartBuf));
    this.movieClipModifiers.forEach((movieClipModifier) =>
      movieClipModifier.encode(smartBuf)
    );
