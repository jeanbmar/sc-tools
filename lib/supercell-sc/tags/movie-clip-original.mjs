...
import MovieClipFrame from './movie-clip-frame.mjs';
import ScalingGrid from './scaling-grid.mjs';
import Tag41 from './tag-41.mjs';
...
class MovieClipOriginal extends Tag {
  ...
  decode(smartBuf) {
    ...
    if (this.tagSignature === 14) {
      ...
      throw new Error(
        `tag TAG_MOVIE_CLIP no longer supported (tag signature ${this.tagSignature})`
      );
    } else {
    ...
    do {
      ...
      switch (tagSignature) {
        ...
        case 5:
          throw new Error(
            `tag TAG_MOVIE_CLIP_FRAME no longer supported (tag signature ${tagSignature})`
          );
        case 11:
  ...
  encode(mainBuf) {
    ...
    if (![14, 3].includes(this.tagSignature)) {
      ...
      this.frameData
        .flat()
        .forEach((spriteFrame) => smartBuf.writeUInt16LE(spriteFrame));
    }
    smartBuf.writeInt16LE(this.displayObjectIds.length);
    this.displayObjectIds.forEach((displayObjectId) =>
      smartBuf.writeInt16LE(displayObjectId)
    );
    if ([12, 35].includes(this.tagSignature)) {
...
export default MovieClipOriginal;
