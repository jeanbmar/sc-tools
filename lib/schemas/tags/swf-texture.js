...
const { scramble, descramble } = require('../../scrambler');
...
class SwfTexture extends Tag {
    ...
    decode(smartBuf) {
        ...
        if ([27, 28, 29].includes(this.tagSignature)) {
            this.pixels = descramble(this.pixels, this.width, this.height);
        }
    }
    ...
    encode(mainBuf) {
        ...
        if (this.pixels.length) {
            ...
            if ([27, 28, 29].includes(this.tagSignature)) {
                this.pixels = scramble(this.pixels, this.width, this.height);
            }
            switch (this.pixelCode) {
            case 2:
            case 9:
